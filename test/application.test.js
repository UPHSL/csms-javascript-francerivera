...
test("missing first name fails validation", () => {
  const resident = makeValidResident({ firstName: "" });
  const validator = new ResidentValidator();
...
test("missing last name fails validation", () => {
  const resident = makeValidResident({ lastName: "" });
  const validator = new ResidentValidator();
  const errors = validator.validate(resident);
...
test("missing address fails validation", () => {
  const resident = makeValidResident({ address: "" });
  const validator = new ResidentValidator();
  const errors = validator.validate(resident);
...
test("invalid contact number fails validation", () => {
  const resident = makeValidResident({ contactNumber: "0917ABC4567" });
  const validator = new ResidentValidator();
  const errors = validator.validate(resident);
...
test("invalid email fails validation", () => {
  const resident = makeValidResident({ email: "juan.example.com" });
  const validator = new ResidentValidator();
  const errors = validator.validate(resident);
...
test("supported resident statuses pass validation", () => {
  ...
  const inactiveResident = makeValidResident({ status: "Inactive" });
  const validator = new ResidentValidator();
...
test("unsupported resident status fails validation", () => {
  const resident = makeValidResident({ status: "Unknown" });
  const validator = new ResidentValidator();
  const errors = validator.validate(resident);
